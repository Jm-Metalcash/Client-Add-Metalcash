...
if (isset($_POST['add_client'])) {
	...
	if (! isset($error)) {
		...
		if (!empty($_POST['email'])) $email = "'".mysql_real_escape_string($_POST['email'])."'"; else $email = "NULL";
		if (!empty($_POST['phone'])) $phone = "'".mysql_real_escape_string($_POST['phone'])."'"; else $phone = "NULL";
		if (!empty($_POST['company'])) $company = "'".mysql_real_escape_string($_POST['company'])."'"; else $company = "NULL";
		if (!empty($_POST['companyvat'])) $companyvat = "'".mysql_real_escape_string($_POST['companyvat'])."'"; else $companyvat = "NULL";
...
<tr><td>Doc. Type/ID/Exp:</td><td><select name="doctype"><option value="1">Carte d'identit�</option><option value="2">Passeport</option><option value="3">Permis</option><option value="4">Autre</option></select><?=$erroridnumber1?><input type="text" name="idnumber" size=20 value="<?=$_POST['idnumber']?>"><?=$erroridnumber2?>
<?=$errorexp1?>
...
<tr><td>Nom / Pr�nom:</td><td><?=$errorname1?><input style="text-transform:uppercase;" type="text" name="name" size=35 value="<?=$_POST['name']?>"><?=$errorname2?> <?=$errorsurname1?><input type="text" name="surname" size=39 value="<?=$_POST['surname']?>"><?=$errorsurname2?></td><td>Ex: DUPONT Jean</td></tr>
<tr><td>Date de naissance:</td><td><?=$errorbirth1?>
...
<?=$errorbirth2?></td><td>&nbsp;</td></tr>
<tr><td colspan="3">&nbsp;</td></tr>
<tr><td>Adresse:</td><td><?=$erroraddress1?><input type="text" name="address" size=81 value="<?=$_POST['address']?>"><?=$erroraddress2?></td><td>Ex: Rue du puit, 12</td></tr>
<tr><td>Localit�:</td><td><?=$errorlocality1?><input type="text" name="cp" size=81 value="<?=$_POST['cp']?>"><?=$errorlocality2?></td><td>Ex: 4800 Verviers</td></tr>
<tr><td>Pays:</td><td><?=$errorcountry1?><input style="text-transform:uppercase;" type="text" name="country" size=81 value="<?=$_POST['country']?>"><?=$errorcountry2?></td><td>Ex: Belgique</td></tr>
<tr><td colspan="3">&nbsp;</td></tr>
<tr><td>E-Mail:</td><td><input type="text" name="email" size=81 value="<?=$_POST['email']?>"></td><td>Ex: contact@metalcash.be</td></tr>
<tr><td>T�l�phone:</td><td><input type="text" name="phone" size=81 value="<?=$_POST['phone']?>"></td><td>Ex: 0032 474 01 12 93</td></tr>
<tr><td>Canal:</td><td><select name="referer"><option value="1">Recherche Internet</option><option value="2">Recommendation d'un ami</option><option value="4">D�j� client</option><option value="5">Autre</option></select></td><td>&nbsp;</td></tr>
<tr><td>Int�r�t:</td><td><select name="interest"><option value="1">Etain</option><option value="2">M�tal argent�</option><option value="3">D3E</option><option value="4">Catalyseurs</option><option value="10">Tout</option></select></td><td>&nbsp;</td></tr>
<tr><td colspan="3">&nbsp;</td></tr>
<tr><td>Soci�t�:</td><td><?=$errorcompany1?><input style="text-transform:uppercase;" type="text" name="company" size=81 value="<?=$_POST['company']?>"><?=$errorcompany2?></td><td><i>Facultatif</i></td></tr>
<tr><td>TVA:</td><td><?=$errorvat1?><input style="text-transform:uppercase;" type="text" name="companyvat" size=81 value="<?=$_POST['companyvat']?>"><?=$errorvat2?></td><td><i>Facultatif</i></td></tr>
</table>
<p>
